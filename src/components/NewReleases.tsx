    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center border-b border-zinc-900 pb-4">
        <div>
          ...
          <p className="text-xs text-zinc-400">
            Uscite recenti nei tuoi generi preferiti: {preferredGenres.join(', ')}
          </p>
        </div>
        <button
          onClick={load}
      ...
      {isLoading ? (
        <div className="py-16 flex flex-col items-center gap-4 text-zinc-500">
          ...
          <p className="text-sm">Ricerca novita in corso...</p>
        </div>
      ) : error ? (
        <div className="py-12 text-center text-zinc-500">
          <p>{error}</p>
          <button onClick={load} className="mt-3 text-xs text-red-400 hover:underline">Riprova</button>
        </div>
      ) : newBooks.length === 0 ? (
        <div className="py-12 text-center text-zinc-500">
          <p>Nessuna novita trovata per i tuoi generi.</p>
        </div>
      ) : (
            ...
            return (
              <div key={idx} className="group relative bg-zinc-900/40 border border-zinc-800 rounded-xl overflow-hidden hover:scale-105 transition duration-300 shadow-md">
                <div className="aspect-[2/3] relative overflow-hidden">
                  ...
                  {owned && (
                    <div className="absolute top-2 left-2 bg-green-600 text-white text-[8px] font-black px-1.5 py-0.5 rounded">
                      GIA LETTO
                    </div>
                  )}
                  {book.publishedDate && (
